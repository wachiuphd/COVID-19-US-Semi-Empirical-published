...
# initial condition
hyperparms.current <- hyperparms.0 + rnorm(n.hyperparms,0,eta.tuning.hyper)
globalparms.current <- globalparms.0 + rnorm(n.globalparms,0,eta.tuning.global)
indivparms.current <- indivparms.0 + rnorm(n.indivparms,0,eta.tuning.indiv)
...
for (iter in 1:niter) {
  ...
  for (j in 1:n.indivparms) { # affects population prior and likelihood
    if (eta.tuning.indiv[j] > 0) { # Don't run if no jumps
      ...
      Roffset.vec.proposed <- 100*inv.logit(indivparms.proposed[n.id+(1:n.id)])
      Tinf.vec.proposed <- rep(globalparms.proposed[1],n.id)
      if (min(Tinf.vec.proposed)<0) Tinf.vec.proposed <- Tinf.vec.current
      sig2err.proposed <- exp(globalparms.proposed[2])
      if (Fixed_n) {
        lnpopprior.proposed <-
          sum(dnorm(indivparms.proposed[n.id+(1:n.id)],
...
# if (Fixed_n) {
#   allparms.coda <- mcmc(allparms[-(1:10),-(1:3)])
# } else {
#   allparms.coda <- mcmc(allparms[-(1:10),-1])
# }
